...
class VerifyCollectorScreen extends StatelessWidget {
  ...
  const VerifyCollectorScreen({super.key, required this.collectorId});

  @override Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: AppColors.canvas,
      appBar: AppBar(backgroundColor: AppColors.canvas, elevation: 0, leading: IconButton(icon: const Icon(Icons.arrow_back, color: AppColors.textPrimary), onPressed: () => Navigator.pop(context)), title: const Text('Verify: Pedro Reyes', style: TextStyle(color: AppColors.textPrimary, fontWeight: FontWeight.w800))),
      body: ListView(padding: const EdgeInsets.symmetric(horizontal: 28), children: [
        const SizedBox(height: 8),
        Row(children: [_DTile('🪪', 'Profile Photo'), const SizedBox(width: 10), _DTile('📄', 'Brgy Clearance')]),
            ...
            return Container(
              ...
              child: Row(children: [
                if (check) ...[
                  Container(
                    width: 18, height: 18,
                    decoration: BoxDecoration(shape: BoxShape.circle, color: isCheck ? AppColors.success : null, border: isUncheck ? Border.all(color: AppColors.divider) : null),
                    child: isCheck ? const Icon(Icons.check, size: 9, color: Colors.white) : null,
                  ),
                  const SizedBox(width: 8),
                ],
    ...
    return Expanded(
      child: Container(
        ...
        child: Stack(children: [
          Center(child: Column(mainAxisSize: MainAxisSize.min, children: [Text(icon, style: const TextStyle(fontSize: 28)), const SizedBox(height: 4), Text(label, style: const TextStyle(fontSize: 10, color: AppColors.textSecondary, fontWeight: FontWeight.w600), textAlign: TextAlign.center)])),
